...
def Update():
    ...
    if dcode.get == "" or dname.get() == "":
        ...
        cursor.execute("UPDATE `department` SET Department_Name=%s WHERE Department_Code=%s VALUES(?,?)",(code1,name1))
        conn.commit()
        dcode.set("")
        dname.set("")
...
Head=Label(frame1,text="Department Updation Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=280,y=20)
...
Submit_button=Button(frame1,text="Submit",font=("Lato",16),bg="#ffa31a",fg="white",command=Update).place(x=220,y=560,width=100,height=40)
