...
def Delete():
    ...
    if scode.get == "" or sname.get() == "" or scapacity.get == "":
        ...
        cursor.execute("DELETE FROM `section` WHERE Section_Name=%s VALUES(?)",(name1))
        conn.commit()
        scode.set("")
        sname.set("")
        scapacity.set("")
...
Head=Label(frame1,text="Section Deletion Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=400,y=20)
