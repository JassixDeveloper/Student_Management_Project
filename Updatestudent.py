    ...
    if username.get() == "" or password.get == "" or email.get() == "" or confirm.get == "":
        ...
        if cursor.fetchone() is not None:
            ...
            if first.get == "" or last.get() == "" or DOB.get() == "" or g.get() == "" or ad.get() == "" or no.get() == "" or s.get() == "" or c.get() == "" or b.get() == "" or se.get() == "":
                ...
                cursor.execute("UPDATE `student` SET Last_Name=%s, DOB=%s, Gender=%s, Address=%s, Contact_No=%s, Shift=%s, Course=%s, Batch=%s, Section=%s WHERE First_Name=%s VALUES(?,?,?,?,?,?,?,?,?,?)",(fname,lname,date1,gender1,address,contact,shift1,course1,bat,section1))
                conn.commit()
                first.set("")
                last.set("")
                DOB.set("")
...
Head=Label(frame1,text="Student Updation Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=350,y=20)
...
#================== Box3 Content
Submit_button=Button(frame1,text="Submit",font=("Lato",16),bg="#ffa31a",fg="white",command=Update).place(x=240,y=685,width=100,height=35)
