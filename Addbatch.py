...
def Register():
    ...
    if bname.get == "" or byear.get() == "" or intake.get == "":
        ...
        cursor.execute('CREATE TABLE IF NOT EXISTS `batch`(Batch_ID integer primary key autoincrement,Batch_Name TEXT,Batch_Year TEXT,Batch_Intake TEXT)')
        cursor.execute("INSERT INTO `batch` (Batch_Name, Batch_Year, Batch_Intake) VALUES(?,?,?)",(name1,year1,intake1))
        conn.commit()
        bname.set("")
        byear.set("")
        intake.set("")
...
Head=Label(frame1,text="Batch Registeration Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=280,y=20)
