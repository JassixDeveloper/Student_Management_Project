...
def Update():
    ...
    if scode.get == "" or sname.get() == "" or scapacity.get == "":
        ...
        cursor.execute("UPDATE `section` SET Section_Name=%s, Section_Capacity=%s WHERE Section_Code=%s VALUES(?,?,?)",(code1,name1,capacity1))
        conn.commit()
        scode.set("")
        sname.set("")
        scapacity.set("")
...
Head=Label(frame1,text="Section Updation Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=350,y=20)
