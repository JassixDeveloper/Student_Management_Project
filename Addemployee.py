    ...
    if username.get() == "" or password.get == "" or email.get() == "" or confirm.get == "":
        ...
        if cursor.fetchone() is not None:
            ...
            if first.get == "" or last.get() == "" or DOB.get() == "" or gender.get() == "" or ad.get() == "" or no.get() == "":
                ...
                cursor.execute('CREATE TABLE IF NOT EXISTS `employee`(Employee_ID integer primary key autoincrement,First_Name TEXT,Last_Name TEXT,DOB TEXT,Gender TEXT,Address TEXT,Contact_No TEXT,Department TEXT)')
                cursor.execute("INSERT INTO `employee` (First_Name, Last_Name, DOB, Gender, Address, Contact_No, Department) VALUES(?,?,?,?,?,?,?)",(fname,lname,date1,gender1,address,contact,dep))
                conn.commit()
                first.set("")
                last.set("")
                DOB.set("")
...
Head=Label(frame1,text="Employee Registeration Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=280,y=20)
...
#================== Box3 Content
Submit_button=Button(frame1,text="Submit",font=("Lato",16),bg="#ffa31a",fg="white",command=Login).place(x=300,y=675,width=100,height=35)
Clear_button=Button(frame1,text="Clear",font=("Lato",16),bg="#ffa31a",fg="white",command=clear).place(x=450,y=675,width=100,height=35)
Back_button=Button(frame1,text="Back",font=("Lato",16),bg="#ffa31a",fg="white",command=back).place(x=600,y=675,width=100,height=35)
Exit_button=Button(frame1,text="Exit",font=("Lato",16),bg="#ffa31a",fg="white",command=Exit).place(x=750,y=675,width=100,height=35)
