...
def Delete():
    ...
    if cname.get == "" or cduration.get() == "" or ccredit.get == "":
        ...
        cursor.execute("DELETE FROM `course` WHERE Course_Name=%s VALUES(?)",(name1))
        conn.commit()
        cname.set("")
        cduration.set("")
        ccredit.set("")
...
Head=Label(frame1,text="Course Deletion Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=400,y=20)
