    ...
    if username.get() == "" or password.get == "" or email.get() == "" or confirm.get == "":
        ...
        if cursor.fetchone() is not None:
            ...
            if first.get == "" or last.get() == "" or DOB.get() == "" or gender.get() == "" or ad.get() == "" or no.get() == "":
                ...
                cursor.execute("DELETE FROM `employee` WHERE First_Name=%s, Last_Name=%s VALUES(?,?)",(fname,lname))
                conn.commit()
                first.set("")
                last.set("")
                DOB.set("")
...
Head=Label(frame1,text="Employee Deletion Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=400,y=20)
...
#================== Box3 Content
Submit_button=Button(frame1,text="Submit",font=("Lato",16),bg="#ffa31a",fg="white",command=Delete).place(x=240,y=675,width=100,height=35)
