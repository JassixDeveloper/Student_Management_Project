...
def Update():
    ...
    if cname.get == "" or cduration.get() == "" or ccredit.get == "":
        ...
        cursor.execute("UPDATE `course` SET Course_Duration=%s, Course_Credit=%s WHERE Course_Name=%s VALUES(?,?,?)",(name1,duration1,credit1))
        conn.commit()
        cname.set("")
        cduration.set("")
        ccredit.set("")
...
Head=Label(frame1,text="Course Updation Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=350,y=20)
