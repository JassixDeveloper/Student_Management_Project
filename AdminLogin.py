...
def Login(event=None):
    ...
    if username.get() == "" or password.get() == "":
        ...
        cursor.execute("SELECT * FROM `new user` WHERE `username` = ? AND `password` = ?", (username.get(), password.get()))
...
Password_entry=Entry(root,bg="white",bd="0",font=("Lato",12),textvariable=password)
Password_entry['show']='*'
...
Login_button=Button(root,text="Login",font=("Lato",16),bg="#ffa31a",fg="white",command=Login).place(x=510,y=600,width=120)
Back_button=Button(root,text="Back",font=("Lato",16),bg="#ffa31a",fg="white",command=Back).place(x=680,y=600,width=120)
Exit_button=Button(root,text="Exit",font=("Lato",16),bg="#ffa31a",fg="white",command=Exit).place(x=850,y=600,width=120)
