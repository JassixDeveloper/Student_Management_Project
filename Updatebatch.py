...
def Update():
    ...
    if bname.get == "" or byear.get() == "" or intake.get == "":
        ...
        cursor.execute("UPDATE `batch` SET Batch_Year=%s, Batch_Intake=%s WHERE Batch_Name=%s VALUES(?,?,?)",(name1,year1,intake1))
        conn.commit()
        bname.set("")
        byear.set("")
        intake.set("")
...
Head=Label(frame1,text="Batch Updation Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=350,y=20)
