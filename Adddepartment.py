...
root.title("DEPARTMENT REGISTERATION FORM")
...
def Register():
    ...
    if dcode.get == "" or dname.get() == "":
        messagebox.showinfo("Error","Please complete the required field!")
    else:
        cursor.execute("CREATE TABLE IF NOT EXISTS `department`(Department_ID integer primary key autoincrement,Department_Code TEXT,Department_Name TEXT)")
        cursor.execute("INSERT INTO `department`(Department_Code, Department_Name) VALUES(?,?)",(code1,name1))
        conn.commit()
        dcode.set("")
        dname.set("")
...
Head=Label(frame1,text="Department Registeration Form",font=("Helvetica",50,"bold"),bg="white",fg="#ff9999").place(x=280,y=20)
...
content_box_label=Label(frame1,text="Department Details",font=("Helvetica",13),bg="white",fg="black").place(x=160,y=188)

box_content1=Label(frame1,text="Department Code",font=("Helvetica",13),bg="white",fg="black").place(x=320,y=300)
box_content1_line=Label(frame1,text="",bg="#737373").place(x=460,y=320,width=355,height=1)
box_content1_entry=Entry(frame1,text="",bd="0",textvariable=dcode)
box_content1_entry.place(x=460,y=300,width=355)

box_content2=Label(frame1,text="Department Name",font=("Helvetica",13),bg="white",fg="black").place(x=320,y=360)
box_content2_line=Label(frame1,text="",bg="#737373").place(x=465,y=380,width=365,height=1)
box_content2_entry=Entry(frame1,text="",bd="0",textvariable=dname)
box_content2_entry.place(x=465,y=360,width=365)
